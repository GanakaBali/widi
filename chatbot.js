...
`;
  ...
  const chatHTML = `
    ...
    <div class="chat-popup" id="chatPopup" role="dialog" aria-modal="true" aria-label="Yasui Tour Chat">
      <div class="chat-header">Widi Bali チャット<button id="closeChat" aria-label="Close chat">✕</button></div>
...
inputContainer.appendChild(toggleQuickBtn);
  ...
  function loadChat(){
    ...
    else addMessage("こんにちは！Widi Bali です🌴 ご質問は日本語でどうぞ。", "bot", false);
  }
  function clearChat(){
    if (confirm("チャット履歴を削除しますか？")){
      ...
      chatBox.innerHTML = "";
      lastDate = "";
      addMessage("こんにちは！Widi Bali です🌴 ご質問は日本語でどうぞ。", "bot", false);
    }
  }
